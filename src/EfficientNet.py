...
class EfficientNet(nn.Module):
    def __init__(self, input_size, model, num_classes=10):
        ...
        width_multi, depth_multi, net_h, dropout_rate = architecture[model]
        ...
        self.mod1 = ConvBlock(input_size, out_channels, kernel_size=3, stride=2, groups=1, dilate=1)
    ...
    def forward(self, x):
        activation1 = self.mod1(x)
        activation2 = self.mod2(activation1)
        activation3 = self.mod3(activation2)
        activation4 = self.mod4(activation3)
        activation5 = self.mod5(activation4)
        activation6 = self.mod6(activation5)
        activation7 = self.mod7(activation6)
        activation8 = self.mod8(activation7)
        activation9 = self.last_feat(activation8)

        x = F.adaptive_avg_pool2d(activation9, (1, 1)).view(-1, self.last_channels)
        if self.training and (self.dropout_rate is not None):
            x = F.dropout(input=x, p=self.dropout_rate,
                          training=self.training, inplace=True)
        out = self.classifier(x)

        return out, activation1, activation2, activation3, activation4, activation5, activation6, activation7, activation8, activation9
