...
class No_teacher:
    def __init__(self):
        ...
        self.train_loader, self.test_loader, self.validation_loader, self.num_classes = dataloaders.transform_data(
            self.dataset,
            M=config.downsample['value'], down=config.downsample['action'])
        self.model_type = config.model_type

        if self.model_type == "rnn":
            self.model = ResNet.WideResNet(depth=config.teacher_rnn['depth'], num_classes=self.num_classes,
            ...
            self.model = EfficientNet.EfficientNet(config.teacher_efficient_net['input_features'],
    ...
    def train(self):
        ...
        self.optimiser = torch.optim.SGD(self.model.parameters(), lr=0.1, momentum=0.9, nesterov=True,
                                         weight_decay=5e-4)
        self.loss_function = torch.nn.CrossEntropyLoss()

        num_epochs = utils.calculate_epochs(self.dataset, config.downsample['action'], self.M)

        scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimiser,
                                                         milestones=[0.3 * num_epochs - 1, 0.6 * num_epochs - 1,
                                                                     0.8 * num_epochs - 1], gamma=0.2)
            ...
            with tqdm(self.train_loader, total=len(self.train_loader), desc='train', position=0, leave=True) as t:
                for curr_batch, batch in enumerate(self.train_loader):
                    ...
                    t.set_postfix(accuracy='{:05.3f}'.format(running_acc / (curr_batch + 1)),
                                  loss='{:05.3f}'.format(running_loss / (curr_batch + 1)))
                    t.update()
    ...
    def validation(self):
        ...
        with torch.no_grad():
            with tqdm(self.validation_loader, total=len(self.validation_loader), desc='validation', position=0,
                      leave=True) as t:
